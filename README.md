...
| 路径 | 说明 |
|------|------|
| `index.html` | 页面骨架：国风卷轴主导航（3×2 六大关 + 终章、墨径 SVG）、各模块视图、底栏 |
...
3. **算学馆** — 数据速算：影子法（人高 / 标杆）、平面镜法、标杆法多模板随机，±0.1 m 判分  
4. **古建测绘** — 互动测绘（Canvas）+ **算经闯关**（《孙子算经》《九章算术》名题）  
5. **鉴误阁** — 误差分析（倾斜 + 风力）  
6. **科举考** — 随堂评价：手电+平面镜测城墙、纸板测树高、台阶折影等  
7. **极限挑战**（终章）— **高阶闯关** + **风力模拟**（手动倾角固定 0°）

## 运行方式

用本地静态服务器打开目录（或直接双击 `index.html`）。无需构建步骤。

首页闯关图为规整三列两行 + 底栏终章；主界面入口不再先做卷轴横向收拢（避免切换中间态整屏只剩窄条），进入子模块时由视图 `opacity` 过渡完成切换感。
